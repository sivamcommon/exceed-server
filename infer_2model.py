...
ENGINE_PATH = "best_11.engine"
DEFECT_ENGINE_PATH = "defect.engine"
...
def draw_filtered_boxes(frame_bgr, result, keep_indices, color=(0, 255, 0)):
    ...
    for i in keep_indices:
        ...
        if cname_l in ("leaf", "spinach"):
            ...
            box_color = (144, 238, 144)  # light green
        else:
            box_color = (0, 0, 255)
...
def update_tracking_stats(
    ...
    # Note: defect classes are collected from the second model crop pass.
    # We intentionally do not add defect IDs from the main model here.
...
def compute_tracking_summary(
    ...
    # Determine which defect classes to count
    if defect_classes is None:
        observed = set()
    ...
    # Total defects = sum of per-class counts (one per leaf per class)
...
def infer_frame_leaf_grouped_tracked(
    ...
    # Keep leaf detections from the first model (include stems for tracking)
    ...
    # Run defect model on each leaf crop and draw its detections
    if keep_indices:
        xyxy = result.boxes.xyxy.cpu().numpy()
        ...
        for i in keep_indices:
            ...
            if crop.size == 0:
                continue
            d_results = defect_model.predict(source=crop, verbose=False, imgsz=imgsz, conf=conf, device=device)
            d_res = d_results[0]
            defect_found = False
            if d_res.boxes is None or len(d_res.boxes) == 0:
                ...
                def d_cls_name(cls_id):
                    if isinstance(d_names, dict):
                        return d_names.get(cls_id, str(cls_id))
                    if isinstance(d_names, list) and 0 <= cls_id < len(d_names):
                        return d_names[cls_id]
                    return str(cls_id)
                ...
                for j in range(len(d_xyxy)):
                    ...
                    if stats is not None and track_ids is not None:
                        ...
                        stats.setdefault("leaf_defects", {}).setdefault(leaf_tid, set()).add(cname)

            # If any defect found for this leaf, redraw leaf box in red
            leaf_label = f"{main_cls_name(cls_ids[i])} {confs[i]:.2f}"
            if defect_found:
                cv2.rectangle(annotated, (ix1, iy1), (ix2, iy2), defect_color, 2)
                cv2.putText(
                    annotated,
                    leaf_label,
                    (ix1, max(0, iy1 - 5)),
